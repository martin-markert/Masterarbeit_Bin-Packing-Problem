...
def test_spatial_positional_encoding(container_matrix = None):
    ...
    assert out.shape == (params.batch_size, seq_len, params.dim_model)                                                      # Shape: [samples, num_boxes, dim_model]
...
def test_transformer_encoder(container_matrix = None):                                                      # TODO: Check, whether all the shapes are chosen correctly
    ...
    assert out.shape == (params.batch_size, seq_len, params.dim_model)                                      # Shape: [batch_size, seq_len, dim_model]
...
def test_transformer_decoder(container_matrix = None):                                                                         
    ...
    assert out.shape == (params.batch_size, env.box_num, params.dim_model)                                  # Shape: [batch_size, box_num, dim_model]
...
def test_position_selection(container_matrix = None):
    ...
    assert bin_encoder_out.shape == (params.batch_size, seq_len, params.dim_model)                          # Shape: [batch_size, seq_len, dim_model]
    assert box_encoder_out.shape == (params.batch_size, env.box_num, params.dim_model)                      # Shape: [batch_size, box_num, dim_model]
    assert position_logits .shape == (params.batch_size, seq_len)                                           # Shape: [batch_size, seq_len] --> Position_Selection does squeeze(-1), so dim_model is lost
...
def test_get_reward_sum(container_matrix = None):
    ...
    agent = Agent(bin_size_x = env.bin_size_x,
                  bin_size_y = env.bin_size_y,
                  learning_rate_actor = params.learning_rate_actor,
                  learning_rate_critic = params.learning_rate_critic
                )
    ...
    buffer = list()
    buffer[:] = trajectory_list
    
    buffer_len = buffer[3].shape[0]
    rewards = buffer[3]
    masks = buffer[4]
    ...
    state_buffer = [torch.as_tensor(array, device = device) for array in buffer[0]]

    value_buffer = [critic([s[i:i + block_size] for s in state_buffer]) for i in range(0, buffer_len, block_size)]

    value_buffer = torch.cat(value_buffer, dim = 0)
    values = value_buffer.cpu().numpy()
    ...
    sum_rewards, advantages = agent.get_reward_sum(buffer_len, rewards, masks, values)
...
'''
    --- Testing the file train.py ---
'''


def test_train_smoke():
    # --- Reduced parameters for quick smoke test ---
    from parameters import Parameters
    params = Parameters()
    params.bin_size_x = 9
    params.bin_size_y = 9
    params.bin_size_z = 100
    params.bin_size_ds_x = 3
    params.bin_size_ds_y = 3
    params.box_num = 2
    params.target_step = 16       # Small amount
    params.batch_size = 2
    params.repeat_times = 1
    params.process_num = 1
    params.reward_scale = 1
    params.break_step = 16        # After short time
    params.load_model = False     # No model to load
    ...
    for i in range(params.process_num):
        p = Process(
            ...
            args = (
                ...
                params.min_factor,
                params.max_factor,
                None                        # Rotation constraints
            )
        )
        p.start()
        workers.append(p)

    for q in action_queue_list:
        q.put(False)

    # try:
    #     # --- Start training ---
    #     train_and_evaluate(params, action_queue_list, result_queue_list)
    #     print("Smoke test for train.py successful – No exceptions.")
    # except Exception as e:
    #     print("Smoke test failed:", e)
    #     raise

     # --- Agent Exploration starten ---
    try:
        with torch.no_grad():
            trajectory_list = agent.explore_environment_multiprocessing(
                action_queue_list,
                result_queue_list,
                params.target_step,
                params.reward_scale,
        ...
        for i, arr in enumerate(trajectory_list):
            print(f"  Array {i} shape: {arr.shape if hasattr(arr, 'shape') else type(arr)}")

    finally:
        # --- Termninate all workers ---
        for p in workers:
            p.terminate()
            p.join()
...
if __name__ == "__main__":
    ...
    # test_bin_encoder()                                                                                # Works, when this one line in network.py is added. See vomment at the bottom
    # test_bin_encoder(container_matrix)                                                                # Works, when this one line in network.py is added. See vomment at the bottom
   
    # test_transformer_encoder()                                                                        # Works, when batch_size == 1
    # test_transformer_encoder(container_matrix)                                                        # Works, when batch_size == 1
    ...
    # test_box_selection_without_plane_features()                                                       # Works not anymore
    # test_box_selection_without_plane_features(container_matrix)                                       # Works not anymore
    ...
    # test_get_reward_sum()                                                                             # Problem with that one line in network.py
    # test_get_reward_sum(container_matrix)                                                             # Problem with that one line in network.py
    ...
    '''
        TODO
        Line
        bin_embedding = bin_embedding.view(-1, bin_embedding.shape[-1]).unsqueeze(0)
        in network.py is a trouble causer

        If added stuff works, that would not oherwise. If deleted the other way round

        Affected tests by thsis line:
        test_bin_encoder
        test_get_reward_sum()
    '''