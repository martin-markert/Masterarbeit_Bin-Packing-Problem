...
class Parameters():
    def __init__(self):
        ...
        '''
        self.break_step    =          2 ** 30                       # Maximum number of total training environment steps after which training stops (termination condition)
        self.process_num   =          4                             # How many parallel processes, each with its own environment? Must not be more than CPU kernels available. step() and reset() are running in the CPU, so this is the important parameter. The model itself is in the GPU but there is only one
